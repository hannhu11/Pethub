      ...
      <div className='max-w-2xl mx-auto px-4'>
        ...
        <div className='bg-white border border-[#592518] rounded-2xl p-6'>
          <div className='flex items-center gap-4 mb-6'>
            <div className='w-16 h-16 rounded-full bg-[#d56756] flex items-center justify-center'>
              <User className='w-8 h-8 text-white' />
            </div>
            <div>
              <h2 className='text-lg'>{profile.name}</h2>
              <p className='text-sm text-[#8b6a61]'>{session.user?.role === 'manager' ? 'Quản trị viên' : 'Khách hàng'}</p>
            </div>
            <button
              onClick={() => setEditing((prev) => !prev)}
              disabled={onboardingRequired || saving}
              className='ml-auto p-2 rounded-xl border border-[#592518] hover:-translate-y-0.5 transition-all'
            >
          ...
          <div className='space-y-4'>
            {[
              ...
              <div key={field.key}>
                <label className='text-sm text-[#8b6a61] mb-1 flex items-center gap-2'>
                ...
                <input
                  ...
                  className={`w-full p-3 rounded-xl border ${
                    editing ? 'border-[#d56756] bg-white' : 'border-[#592518]/20 bg-[#f4ece4]'
                  } focus:outline-none focus:ring-2 focus:ring-[#d56756]`}
                />
              </div>
            ))}
          </div>
          ...
          {editing ? (
            <button
              ...
              className='w-full mt-6 py-3 rounded-xl bg-[#d56756] text-white hover:-translate-y-0.5 transition-all border border-[#592518] disabled:opacity-60 disabled:hover:translate-y-0'
            >
        ...
        <div className='grid grid-cols-2 gap-4 mt-6'>
          ...
          <Link
            ...
            className='bg-white border border-[#592518] rounded-2xl p-4 hover:-translate-y-0.5 transition-all flex items-center gap-3'
          >
            <Calendar className='w-5 h-5 text-[#c75b4c]' />
    ...
    <div className='py-12'>
      <div className='max-w-5xl mx-auto px-4'>
        ...
        <div className='grid md:grid-cols-2 gap-6'>
          {pets.map((pet) => (
            <div key={pet.id} className='bg-white border border-[#592518] rounded-2xl overflow-hidden'>
              <div className='flex p-4 gap-4'>
                <div className='w-24 h-24 rounded-2xl overflow-hidden border border-[#592518] flex-shrink-0'>
              ...
              <div className='grid grid-cols-3 border-t border-[#592518]/10 text-sm'>
                <button type='button' onClick={() => openDetail(pet)} className='py-3 text-[#d56756] hover:bg-[#d56756]/5'>
                  <Eye className='w-4 h-4 inline-block mr-1' />
                  Chi tiết
                </button>
                <button type='button' onClick={() => void openMedical(pet)} className='py-3 text-[#c75b4c] hover:bg-[#c75b4c]/5 border-x border-[#592518]/10'>
                  <FileText className='w-4 h-4 inline-block mr-1' />
                  Bệnh án
                </button>
                <Link to={`/customer/digital-card/${pet.id}`} className='py-3 text-[#592518] hover:bg-[#f4ece4] text-center'>
                  Thẻ
                </Link>
              </div>
            </div>
        ...
        {loading ? <p className='text-sm text-[#8b6a61] mt-4'>Đang tải thú cưng...</p> : null}
        {!loading && pets.length === 0 ? (
          <div className='text-sm text-[#8b6a61] mt-4 rounded-xl border border-[#592518]/20 bg-white p-4'>
          ...
          <div className='fixed inset-0 z-50 bg-black/30 flex items-center justify-center p-4' onClick={closeDetail}>
            <div className='w-full max-w-xl bg-[#faf8f5] border border-[#592518] rounded-2xl p-5 max-h-[80vh] overflow-y-auto' onClick={(event) => event.stopPropagation()}>
              ...
              <div className='grid grid-cols-2 gap-2 text-sm'>
                <div className='rounded-xl border border-[#592518]/15 p-2'>Giới tính: {selectedPet.gender || 'Chưa cập nhật'}</div>
                <div className='rounded-xl border border-[#592518]/15 p-2'>Cân nặng: {selectedPet.weightKg ? `${selectedPet.weightKg}kg` : 'Chưa cập nhật'}</div>
                <div className='rounded-xl border border-[#592518]/15 p-2'>Nhóm máu: {selectedPet.bloodType || 'Chưa cập nhật'}</div>
                <div className='rounded-xl border border-[#592518]/15 p-2'>Microchip: {selectedPet.microchipId || 'Chưa cập nhật'}</div>
              </div>

              <div className='mt-4'>
                <h3 className='text-sm text-[#592518]' style={{ fontWeight: 700 }}>
                  Bệnh án gần đây
                </h3>
                {loadingRecords ? <p className='text-xs text-[#8b6a61] mt-2'>Đang tải bệnh án...</p> : null}
                {!loadingRecords && visibleRecords.length === 0 ? <p className='text-xs text-[#8b6a61] mt-2'>Chưa có bệnh án.</p> : null}
  ...
  return (
    <div className='py-12'>
      <div className='max-w-3xl mx-auto px-4'>
        ...
        {error ? <div className='mb-4 rounded-xl border border-red-300 bg-red-50 p-3 text-sm text-red-700'>{error}</div> : null}
        {loading ? <p className='text-sm text-[#8b6a61] text-center'>Đang tải thẻ digital...</p> : null}
          ...
          <>
            ...
            <div className='max-w-2xl mx-auto mt-4 flex gap-3'>
              <button
                ...
                className='flex-1 inline-flex items-center justify-center gap-2 px-4 py-2.5 border border-[#592518] rounded-xl bg-white hover:-translate-y-0.5 transition-all'
              >
